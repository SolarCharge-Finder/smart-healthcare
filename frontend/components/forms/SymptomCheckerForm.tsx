...
export default function SymptomCheckerForm() {
  ...
  const summaryText = useMemo(
    () => removeNextStepsFromSummary(result?.analysis),
    [result?.analysis],
  );
    ...
    <Card title="AI Clinical Triage Assistant">
      <form className="space-y-6" onSubmit={onSubmit}>
        <div className="rounded-2xl border border-sky-100 bg-gradient-to-r from-sky-50 via-white to-cyan-50 p-5 text-base text-slate-700">
          ...
          <div className="mt-1">
            Provide symptom details to generate a structured triage response with urgency and likely
            conditions.
          </div>
        </div>
        ...
        <label className="block space-y-1">
          ...
          <textarea
            ...
            maxLength={2000}
          />
          <div className="text-xs text-slate-500">{symptoms.length}/2000 characters</div>
        </label>
          ...
          <div className="space-y-5 rounded-2xl border border-slate-200 bg-slate-50 p-6 text-base text-slate-800">
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`rounded-full border px-3 py-1 text-sm font-semibold ${urgencyStyles.badge}`}
                  >
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-5">
                ...
                <p className="mt-3 text-sm text-slate-700">
                  {getConfidenceMessage(confidencePercent)}
                </p>
              </div>
            </div>
            ...
            {result.recommendedSpecialty && (
              <div>
                ...
                <p className="mt-1 rounded-xl border border-slate-200 bg-white p-4 text-base text-slate-800">
                  {result.recommendedSpecialty}
                </p>
              </div>
            )}
            ...
            <details className="rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-600">
              ...
              <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-4">
                ...
                {result.correlationId && (
                  <div className="break-all md:col-span-2 xl:col-span-4">
                    Correlation ID: {result.correlationId}
                  </div>
                )}
              </div>
